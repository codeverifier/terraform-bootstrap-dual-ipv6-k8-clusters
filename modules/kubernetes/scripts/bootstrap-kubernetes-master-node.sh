...
# Install calico
sed -i -r "s@%%POD_SUBNET_CIDR%%@$POD_SUBNET_CIDR@" /tmp/calico.yaml
sed -i -r "s@%%VPC_CIDR%%@$VPC_CIDR@" /tmp/calico.yaml
...
kubeadm kubeconfig user --client-name $CLUSTER_NAME-admin --config /tmp/kubeadm.yaml > $KUBECONFIG_IP_OUTPUT
...
# Remove any hanging APT packages
apt autoremove